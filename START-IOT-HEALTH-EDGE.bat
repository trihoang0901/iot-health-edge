...
if defined COM_PORT (
    ...
    powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$root=$env:IOT_HEALTH_ROOT; $culture=[Globalization.CultureInfo]::InvariantCulture; $styles=[Globalization.DateTimeStyles]([int][Globalization.DateTimeStyles]::AssumeUniversal -bor [int][Globalization.DateTimeStyles]::AdjustToUniversal); $started=[DateTimeOffset]::Parse($env:UPLOAD_STARTED_UTC,$culture,$styles); $text=[IO.File]::ReadAllText((Join-Path $root 'firmware\health-node\include\secrets.h')); $quote=[char]34; $match=[regex]::Match($text,'(?m)^\s*#define\s+DEVICE_ID\s+'+$quote+'([^'+$quote+']+)'+$quote); if(!$match.Success){exit 1}; $base='http://127.0.0.1:8000/api/v1/devices/'+[uri]::EscapeDataString($match.Groups[1].Value); $deadline=(Get-Date).AddSeconds(30); do{try{$device=Invoke-RestMethod -Uri $base -TimeoutSec 3; $latest=Invoke-RestMethod -Uri ($base+'/latest') -TimeoutSec 3; $received=[DateTimeOffset]::Parse($latest.received_at,$culture,$styles); $schema=if($latest.schema){$latest.schema}else{$latest.schema_version}; if($device.online -eq $true -and $received -ge $started -and $schema -eq 'health.telemetry.v2' -and $latest.system.fw -eq '0.2.2'){exit 0}}catch{}; Start-Sleep -Seconds 2}while((Get-Date) -lt $deadline); exit 1"
    if errorlevel 1 (
